...
const RegistrationTypeWrapper = styled("div")`
  aspect-ratio: 1.08 / 1;
  cursor: pointer;
  position: relative;
  display: flex;
...
const RegistrationRadio = styled.input.attrs({ type: "radio" })`
  pointer-events: none;
  user-select: none;
  position: absolute;
  top: 0;
...
function RegistrationType({ id, type, text, ...props }) {
  return (
    <RegistrationTypeWrapper>
      <RegistrationRadio id={id} type={type} text={text} {...props} />
      <RegistrationLabel htmlFor={id} />
